...
public class HistoryController implements ManageProjectsController {
	...
	public void populateProjects(String historyContentString) {
		...
		if (historyContentString.contains("~")) {
			numberOfSeparators = countHowManySeparators(historyContentString);
			...
			for (int i = 0; i < numberOfProjectsInTheFile; i++) {
				...
				setSplittedParts(numberOfSeparators, splitParts, historyContentString, i);

				addProjectsIntoHistoryOfProjectsArray(splitParts);
			}
		}
		else {
	...
	void setSplittedParts(int numberOfSeparators, ArrayList<String> splitParts, String historyContentString, int iteratorNumber){
		String[] parts = historyContentString.split("~");

		for (int j = 0; j < separatorsPerProject; j++) {
			splitParts.add(j, parts[j + (separatorsPerProject * iteratorNumber)]);
		}
	}
	
	int countHowManySeparators(String historyContentString){
		int numberOfSeparators = 0;
	...
	void addProjectsIntoHistoryOfProjectsArray(ArrayList<String> splitParts){
